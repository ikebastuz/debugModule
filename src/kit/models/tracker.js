...
class TrackerApi {
  ...
  feed(cvData, keepOnly = []) {
    const calc_start = performance.now();

    if (keepOnly.length > 0) {
      Object.keys(this.persons).map((hash) => {
        if (!keepOnly.includes(hash)) delete this.persons[hash];
      });
    }
  ...
  genSnapShot(snapshot, personId) {
    if (this.addPalms) {
      snapshot = this.injectPalms(snapshot);
    }
  ...
  injectPalms(snapshot) {
    if (!snapshot[3] && !snapshot[6]) return snapshot;

    snapshot = this.predictPalm(snapshot, 4);
    snapshot = this.predictPalm(snapshot, 7);

    return snapshot;
  }

  predictPalm(snapshot, handIdx) {
    if (snapshot[handIdx]) {
      snapshot[handIdx * 11] = {
        x:
          snapshot[handIdx].x +
